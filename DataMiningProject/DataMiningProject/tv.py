...
while current_k<10:
    ...
    #if you give N, then just stopo when oyou hit x number of paragraphs
    #N = 18,118
    ...
    for file_name in list_of_file_names:
        ...
        for paragraph in paragraphs:
            ...
            if(current_count<start_count+test_count and current_count>=start_count):
                ...
                if not all_places[paragraph_index].isdigit():
                    N_places += 1
                if not all_topics[paragraph_index].isdigit():
                    N_topics += 1
                #print(current_count)
                ...
                #arti
                places = all_places[paragraph_index].split(',')
                for p in places:
                    if not p.isdigit() and not p.isspace() and p!='':
                        #put into distinct places list
                        if p not in distinct_places:
                            distinct_places.append(p)
                            distinct_places_and_empty.append(p)
                    ...
                    elif not p.isspace() and p!='':
                        distinct_places_and_empty.append(p)

                #arti
                topics = all_topics[paragraph_index].split(',')

                for t in topics:
                    if not t.isdigit() and not t.isspace() and t!='':
                        #put into distinct places list
                        if t not in distinct_topics:
                            distinct_topics.append(t)
                            distinct_topics_and_empty.append(t)
    ...
    #
    # for k,v in distinct_topics_count.items():
    #     print(k,v)
    # print("\n\n\n\n\n\n\n")
    # for k,v in distinct_places_count.items():
    #     print(k,v)
            ...
            if w in new_uncommon_words:
                ...
                if w in new_uncommon_words:
                    ...
                    if(place in distinct_places_count):
                        uncommon_words_count_p[place][w] = (c/distinct_places_count[place])
            ...
            if w in new_uncommon_words:
                if w in new_uncommon_words:
                    if topic in distinct_topics_count:
                        uncommon_words_count_t[topic][w]=(c/distinct_topics_count[topic])
                    else:
    ...
    print("\n\n\n\n\n\n\n\n")
    new_uncommon_words_file.close()
    file.close()
    current_k += 1
    start_count = math.floor(current_k*test_count)
